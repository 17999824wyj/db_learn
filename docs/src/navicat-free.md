...
但是在本节, 将给出**并非**参照上述解决方案进行解决的流程, 而是通过直接修改原始的 sql 脚本。如果需要复现, 自然可以, 当然也可以直接使用我制作的课程资料包的补丁包 `apple` 进行操作。
...
后续看了下表格内容，发现 `comments` 和 `customer` 这两张表格的数据存在异常。两张表格都是空的。

为了 debug，在 mysql 内执行了下面命令：
...
然后查看 `D:\mysql-log-.txt` 文件，发现 `comments` 和 `customer` 的数据在插入过程中会因为不满足约束的原因，导致插入失败。

但问题是，为什么破坏了约束呢？

为了调试，在 mysql 内直接执行一行查询：

```
mysql> INSERT INTO `comments` (`goodid`, `custid`, `stars`, `remark`, `feeling`, `submtime`) VALUES ('12560557', 1, 5, '非常好希望价格更优惠！知识图谱目前介绍最全面的一本书！', '好评', '2020-04-08 09:26:00');
ERROR 3819 (HY000): Check constraint 'comments_chk_1' is violated.
```

发现 `comments_chk_1` 这个约束条件被违反了。然后采取下面命令查看此约束的具体内容：
...
| comments | CREATE TABLE `comments` (
  `goodid` char(12) NOT NULL,
  `custid` int NOT NULL,
  `stars` int DEFAULT NULL,
  `remark` varchar(512) DEFAULT NULL,
  `feeling` char(4) DEFAULT NULL,
  `submtime` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`goodid`,`custid`),
  CONSTRAINT `comments_chk_1` CHECK ((((`feeling` = _gbk'好评') and (`stars` >= 4)) or ((`feeling` = _gbk'中评') and ((`stars` = 3) or (`stars` = 2))) or ((`feeling` = _gbk'差评') and (`stars` = 1))))
) ENGINE=InnoDB DEFAULT CHARSET=gbk |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
1 row in set (0.00 sec)
```

所以，可能问题出现在对 `_gbk('好评')` 和 `好评` 的匹配机制上。

然后，最诡异的部分来了：依次执行下面的命令：

```mysql
mysql> select hex(_gbk'好评');
+-----------------+
| hex(_gbk'好评') |
+-----------------+
| BAC3C6C0        |
...
mysql> INSERT INTO `comments` (`goodid`, `custid`, `stars`, `remark`, `feeling`, `submtime`) VALUES ('12560557', 1, 5, '非常好希望价格更优惠！知识图谱目前介绍最全面的一本书！', 0xBAC3C6C0, '2020-04-08 09:26:00');
ERROR 3819 (HY000): Check constraint 'comments_chk_1' is violated.
mysql>
```

可以看到，我把 `_gbk('好评')` 的值直接插入到 `feeling` 字段中，依然违反了约束条件。这？？？？

所以，我就没有办法了...这 mysql 也没有相关的调试功能，现在属于是吧，一个 `if (a == 1)` 出错了，然后我让他判断 `if (1 == 1)`，结果也出错了，这我也没办法啊...
